...
- [ ] 左侧导航包含 `总览 / 清理 / 计划任务 / 配置 / MCP 检索 / 日志`
- [ ] 顶部状态条可见，能显示当前页面、任务状态、活动状态、日志目录
- [ ] 底部活动抽屉默认收起
- [ ] 点击 `显示活动抽屉` 后能展开，再次点击可收起
- [ ] 执行任意动作后，活动抽屉中会记录 `START / SUCCESS / FAILED`
...
- [ ] 总览页能显示最近清理摘要
- [ ] 总览页能显示 MCP 摘要
- [ ] 总览页的 `立即预览` 可触发 dry-run
- [ ] 总览页的 `立即清理（管理员）` 会触发管理员清理链路
- [ ] 总览页的 `安装任务（管理员）` 或 `重装任务（管理员）` 可触发管理员任务链路
- [ ] 总览页的 `扫描 MCP` 可触发 MCP 刷新

## 清理页

- [ ] 点击 `立即预览` 后，表格可展示候选 suites
- [ ] 表格列包含 `Suite / 分类 / Root PID / 进程数 / 创建时间 / 动作`
- [ ] 详情面板能展示：
  - [ ] 判定原因
  - [ ] 风险提示
  - [ ] 进程树
  - [ ] 命令摘要
- [ ] 点击 `复制结果` 后，剪贴板中包含表格头和 suite 数据
- [ ] 点击 `导出结果` 后，`%LOCALAPPDATA%\CodexSubMcpManager\exports\cleanup-suites.tsv` 会生成
- [ ] 非管理员状态点击 `立即清理（管理员）` 时会弹出 UAC
- [ ] 管理员确认后，如存在超额 orphan suite，详情和摘要中能看到真实 `killed pid=...`
- [ ] 管理员拒绝或执行失败时，顶部活动状态和活动抽屉会记录失败
...
- [ ] `scan mcp --format json` 输出顶层包含 `configured` 与 `installed_candidates`
